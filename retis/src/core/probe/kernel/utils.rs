    ...
    F: Fn(&Symbol, &HashSet<ProbeOption>) -> Result<bool>,
{
    ...
    // ftrace cannot be applied to wildcards matching multiple symbols as nested
    // ftrace probes can silently drop events: the inner kretprobe would close
    // the window before the outer one completes.
