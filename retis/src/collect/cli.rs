...
- ftrace: allows reporting events from functions called within the probe target, even those not reporting events under normal conditions (e.g. no known type in their arguments). Those functions should be explicitly probed too (\"--probe\"). It's only valid for kprobes on functions with a struct sk_buff pointer in their arguments; wildcards are not supported. A matching kretprobe is automatically added to close the tracking window. See the documentation for further details about the option.
